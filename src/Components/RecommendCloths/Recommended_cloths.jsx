...
import bg1 from '../../recommend_page.png'
import { Link } from 'react-router-dom'
import Cloths from '../Cloths/Cloths'
import axios from 'axios'

export default function Recommendedcloths(props) {
  ...
  async function changeImage(e) {
    ...
    let {data} = await axios.post(`http://54.85.241.201:8000/recommend_clothes`,formData);
    if (data) {
      setIsLoading(false);
      console.log(data.recommendations);
      setClothes(data.recommendations)
    }
            ...
            <div className='d-flex flex-column h-75 w-75 mt-5 w-lg-50 justify-content-center '>
                <div className='desc d-flex h-100 w-100 flex-column justify-content-center px-3 align-items-center'>
                    <div className='d-flex flex-column align-items-center'>
                        <h2 className='my-3 titles' style={{letterSpacing: "2px"}} >Cloths Recommendition</h2>
                ...
                <div className='d-flex flex-column second-bg rounded-3 col-md-5 col-12 flex-wrap justify-content-center align-items-center'>
                          <h3 className='titles main-color fs-1'>Recommend Clothes</h3>
                          <div className='d-flex rounded-3  col-12 flex-wrap justify-content-center align-items-center'>
                              {clothes.map((clothe)=>{
                                          return(
                                          <div className='col-6  d-flex justify-content-center'><img src={"http://54.85.241.201:8000/"+clothe.url} className=' clothImg  m-3 rounded-2' alt="" srcset="" /></div> 
                                      )
                                      })}
                          </div>
                        </div>
